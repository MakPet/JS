...
/*
    ...
    BigInt - числах не может быть больше 16 цифр, но если надо, то ставим b в конце, целыми числами,
    важно.
...
/*
    ...
    2. Численное преобразование в матем функциях и выражениях, например, когда операция деления
    применяется не к числу. Но с + такое уже вроде не пройдет
...
/*
    ...
    У преинкримента приоритет выше, поэтому a сначала увеличится и станет 2, а затем присвоится c,
    которая тоже станет 2. У постинкримента приоритет тоже выше, но сначала d присвоится b, а затем
    b увеличится будет 2.
...
/* Конструкция "switch"
    ...
    Заменяет несколько if, синтакисис: есть как минимум один блок case и необязательное значение
    default.
    ...
    Если соответствие установлено – switch начинает выполняться от соответствующей директивы case и
    ...
    Сравнит последовательно со всеми case, с 3 совпадений нет, проигнорируется. С 4 уже есть.
    Выполнится до ближайшего break. Если бы break не было, то дальше бы пошло вниз

    switch и case допускают любые значения в качестве аргументов
    ...
    Тут a преобразует в число и сравнит с b + 1, строгое равенство будет истино

    Несколько вариантов case, которые выполняют одинаковый код, можно групировать
    ...
    switch (a) {
        ...
        case 3 :
        case 5 :
            alert("Вы учили матиматеку?");
            break;
        default :
            alert("Резульат выглядит странно")

        Если 3 истино, то наченается выполнение со строки case 3: и продолжается
        до ближайшего break
        Если 3 ложно, то дальше проверяется 5 и тд.
    }

    Типы тут очень важны, т.к. сравнения у нас строгие!
	...
	switch (arg) {
		...
		case 3 :
			alert("Никогда не выполнится, так как prompt вернет строку 3");
			break;	

		default :
			alert("Какое-то другое значение");
	}
...
/*  Функции
    ...
    Используются, если надо повторять одно и тоже действие во многих частях программы, не нужно писать
    один и тот же код во многих местах/

    C тремя встроеными функциями мы уже знакомы, но можно ОБЪЯВИТЬ свои
    ...
    showMessage - имя ф-ии
    () - список параметров
    {} - тело ф-ии

    Теперь вызвать функцию можно с помощью showMessage() ВЫЗОВ выполняет код функции.

    Локальные переменные объявленые внутри функции видны ТОЛЬКО внутри этой функции!

    Но! У функции ЕСТЬ доступ к внешним переменным!
    Причем доступ полный, она может изменять их значения. Тут уже интересно
    ...
    Т.е значение до вызова ф-ии не изменится. Внешние переменные используются, только если в
    ф-ии нет локальных.

    Если же есть одноименная внешняя переменная, то локальная проигнорируется.
    ...
    Заметка: переменные объявленые снаружи всех ф-ий - глобальные. Глобальные видимы для любой ф-ии,
    только если их не перекрывают.
    Но лучше все таки их использование сводить к мимнимуму.

    !Параметры!

    Можем передать любую информацию ВНУТРЬ ф-ии используя параметры
    ...
    Когда мы вызываем ф-ию, то переданные значения копируются в ЛОКАЛЬНЫЕ ПЕРЕМЕННЫЕ from and text.
    Затем они уже у нас используются в теле ф-ии.

    Можем сделать что-то такое:
    ...
    let from = "Аня" // Объявляем глобальную переменную и передаем ее ф-им, затем изменим ее, но
    //изменение не будет видно снаружи функции, только ВНУТРИ!

   showMessage(from, "здарова");

    alert( from );

    !Параметры по умолчанию!

    Если один из параметров не задан, то вместно него будет undefined, а не ошибка.

    showMessage("Аня"); // Аня: undefined (*)

    Если хотим задать параметру значение по умолчанию, то нам нужно поставить = после него и знач
    ...
    В данном случае это сторока, но межет быть и более сложное вражение, например еще одна функция.

    Каждый раз будет вычисляться параметр по умолчанию, пока не будет задан соответствующий параметр
    Раньше JS не поддерживал параметры по умолчанию, но были способы обхода.
    ...
    Мы объявляем переменную result и вызываем функцию с параметрами 1, 2
    В функции выполнение доходит до дерективы return, выполняется, и возвращается.

    Директива return может находится в любом месте функции, как только выполнение доходит до нее,
    функция останавливается, значение возвращается.

    Несколько вызовов return
    ...
    Использование без значение может привести к немедленному выходу из ф-ии
    ...
    Тут в примере выше, если checkAge будет false, то alert не выведется

    !!Если у функции нет return или он пустой, то она ничего не возвращает, т.е
    ...
    alert( example() === undefined ); //Тоже true

    !! НИКОГДА НЕ добавлять пернос строки сразуже после return, автоматически поставится ; и вернется
    пустое значение

    return;
        (some + long + expression + or + whatever * f(a) + f(b))

    Нужно начать выражение на той же строки или добавить там скобку, или же все сразу
    ...
    Функция - действие, начинать с глагола, описывать кратко и емко

    Ниже префиксы

    Ф-ии, начинающиеся с "show" обычно что-то показывают.
    "get…" – возвращают значение,
    "calc…" – что-то вычисляют,
    "create…" – что-то создают,
    "check…" – что-то проверяют и возвращают логическое значение, и т.д.

    Одна функция и ОДНО действие, то, что явно подразумевает ее название

    Несколько плохих перемеров

    getAge – будет плохим выбором, если функция будет выводить alert с возрастом
    (должна только возвращать его).

    createForm – будет плохим выбором, если функция будет изменять документ, добавляя форму в него
    (должна только создавать форму и возвращать её).

    checkPermission – будет плохим выбором, если функция будет отображать сообщение с текстом
    "доступ разрешён/запрещён"
    (должна только выполнять проверку и возвращать её результат).

    Сверхкороткие имена ф-ий используются в фремворках, библиотеках, мне они пока не нужны.
*/